...
;(function(ztree) {
  ...
  let makeNodeLineClassEx = ztree._z.view.makeNodeLineClassEx;
  let mreplaceSwitchClass = ztree._z.view.replaceSwitchClass;
  let makeNodeIcoClass    = ztree._z.view.makeNodeIcoClass;
  let replaceIcoClass     = ztree._z.view.replaceIcoClass;
  let asyncNode           = ztree._z.view.asyncNode;


  function restore() {
    ztree._z.view.makeNodeLineClassEx = makeNodeLineClassEx;
    ztree._z.view.replaceSwitchClass  = mreplaceSwitchClass;
    ztree._z.view.makeNodeIcoClass    = makeNodeIcoClass;
    ztree._z.view.replaceIcoClass     = replaceIcoClass;
    ztree._z.view.asyncNode           = asyncNode;
    return ztree;
  }
  ...
  function beautify({
    ...
    ztree._z.view.asyncNode = function(setting, node, isSilent, callback) {
      ...
      if (node) {
        $("#" + node.tId + "_ico").addClass(ZTREE_ICON_ASYNC)
      }
      return result;
    }
    ...
    ztree._z.view.replaceSwitchClass = function(node, obj, newName) {
      mreplaceSwitchClass(node, obj, newName);
      switch (newName) {
